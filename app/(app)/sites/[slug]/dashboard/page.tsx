...
export default async function SiteDashboardPage({
  ...
  const [adSlice, ga4Day] = await Promise.all([
    getAdDashboardSlice(site.id, reportDate),
    getGa4Day(site.id, reportDate),
  ])
  ...
  return (
    <>
      <TopBar date={reportDate} subtitle={site.name} siteSlug={site.slug} />
      <div className="mx-auto max-w-[1180px] px-8 py-7">
        <div className="flex flex-col gap-6">
          ...
          <Panel title="渠道表现对比" hint="本站点 live connectors · 预算超标标黄">
          ...
          <p className="pt-1 text-center text-[12px] text-[var(--color-ink-faint)]">
            AMD · {site.slug} · 渠道 KPI / GA4 按站点实时拉取 · 凭证站内配置
          </p>
        </div>
      </div>
    </>
